...
def get_close(paper_name: str, date: str):
    print(date)
    cur = conn.execute("SELECT close FROM (SELECT MAX(id) as max_id FROM StockData WHERE datetime <= ? AND paper_name = ? GROUP BY paper_name) AS MAX_IDS INNER JOIN StockData ON StockData.id = MAX_IDS.max_id",
                       (date, paper_name))
    row = cur.fetchone()
